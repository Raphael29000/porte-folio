...
public class ViewPersonnel extends JPanel{
	...
	public ViewPersonnel() {
		...
		JButton gestEqu =new JButton("COUILLE !");
	...
	private void changePersonelValue()
	{
		for(Personnel p : App.getEtablissement().getListPersonnel()) {
          if( (p.getIdentifiant().equals( table.getModel().getValueAt(table.getSelectedRow(), 5)) )
           && (p.getNom().equals( table.getModel().getValueAt(table.getSelectedRow(), 0)) )
           && (p.getPrenom().equals( table.getModel().getValueAt(table.getSelectedRow(), 1)) )
           ){
              selectedPersonnel = p;
          }
      }
